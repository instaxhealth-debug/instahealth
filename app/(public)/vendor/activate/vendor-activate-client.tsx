...
export default function ActivateVendorClient() {
  ...
  const [vendorInfo, setVendorInfo] = useState<{ email: string; businessName?: string | null } | null>(null);
  ...
  useEffect(() => {
    ...
    const loadInvite = async () => {
      try {
        const res = await fetch(`/api/vendor/activate?token=${encodeURIComponent(token)}`);
        const result = await res.json();
        if (!res.ok) {
          setError(result.error || "Invalid invite link");
          return;
        }
        setVendorInfo({ email: result.email, businessName: result.businessName });
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const email = vendorInfo?.email || result?.vendor?.email || "";
      const redirectUrl = `/vendor/login?email=${encodeURIComponent(email)}&activated=1`;
      ...
      <div className="container mx-auto max-w-md px-4 py-16">
        <Card className="border-green-200 bg-green-50">
          ...
          <CardContent className="space-y-4 text-center">
            <p className="text-green-700">Welcome to InstaHealth Vendors!</p>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              {vendorInfo?.email && (
                <div className="space-y-2">
                  ...
                  <div className="w-full rounded border px-3 py-2 text-sm bg-gray-50 text-gray-700">
                    {vendorInfo.email}
                  </div>
                </div>
              )}
